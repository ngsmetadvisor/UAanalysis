...
#    {'id':'CYXE', 'name':'Saskatoon',              'lat':52.17, 'lon':-106.68, 'wmo':'71866'},
  ...
  #  {'id':'OTZ',  'name':'Kotzebue',               'lat':66.87, 'lon':-162.63, 'wmo':'70133'},
   ...
   # {'id':'NHB',  'name':'St Paul Island',         'lat':57.15, 'lon':-170.22, 'wmo':'70308'},
    ...
    {'id':'TFX', 'name':'Great Falls MT',         'lat':47.46, 'lon':-111.39, 'wmo':'72776'},
...
'style="font-size:10px;padding:2px 8px;cursor:pointer;border:1px solid #aaa;border-radius:3px;background:#4a7fc1;color:#fff">-- 00Z</button>'
    ...
    '<button id="btn-ua-avail" onclick="window.open(\'ua_availability.html\',\'_blank\')" '
    'style="font-size:10px;padding:2px 8px;cursor:pointer;border:1px solid #1a3a6a;border-radius:3px;background:#1a3a6a;color:#fff;font-weight:bold;">&#128225; UA Radiosonde Availability &#8599;</button>'
...
gh_trigger_html = (
    ...
    'min-height:12px;"></div>'
    '<div id="gh-dispatch-link" style="display:none;margin-top:4px;">'
    '  <a href="https://github.com/' + GH_REPO + '/actions" target="_blank" '
    '  style="font-size:10px;color:#1a4a8a;font-family:Courier New,monospace;'
    '  text-decoration:underline;">&#8599; View Actions on GitHub</a>'
    '</div>'
    '</div>\n'
...
'    if (!el) { setTimeout(_restoreLastRun, 300); return; }\n'
'    el.textContent = "Last run: checking…";\n'
'    fetch(\n'
'      "https://api.github.com/repos/" + _GH_REPO +\n'
'      "/actions/workflows/" + _GH_WORKFLOW + "/runs?per_page=1&branch=" + _GH_BRANCH,\n'
'      { headers: { "Accept": "application/vnd.github+json",\n'
'                   "X-GitHub-Api-Version": "2022-11-28" } }\n'
'    ).then(function(r){ return r.json(); }).then(function(data) {\n'
'      var run = data.workflow_runs && data.workflow_runs[0];\n'
'      if (!run) { el.textContent = "Last run: unknown"; return; }\n'
'      var dt = new Date(run.updated_at);\n'
'      var pad = function(n){ return String(n).padStart(2,"0"); };\n'
'      var months = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];\n'
'      var lbl = months[dt.getUTCMonth()]+" "+pad(dt.getUTCDate())+" "\n'
'              + pad(dt.getUTCHours())+":"+pad(dt.getUTCMinutes())+"Z"\n'
'              + " (" + (run.conclusion || run.status) + ")";\n'
'      el.textContent = "Last run: " + lbl;\n'
'      localStorage.setItem("gh_last_run", lbl);\n'
'    }).catch(function() {\n'
'      var saved = localStorage.getItem("gh_last_run");\n'
'      if (saved) el.textContent = "Last run: " + saved + " (cached)";\n'
'      else el.textContent = "Last run: unavailable";\n'
'    });\n'
'  }\n'
...
'})();\n'
   ...
   '    document.getElementById("gh-spin").style.display = "none";\n'
    ...
    '      var linkBar = document.getElementById("gh-dispatch-link");\n'
    '      if (linkBar) linkBar.style.display = "block";\n'
    '    } else {\n'
...
try:
    ...
    # ── button wires directly to ua_availability.html (window.open) ──────────
    # No overlay panel injected — the standalone file is the UI.
    print(f'✓ UA Radiosonde station data ready ({len(_ua_stations_data)} stations) → button opens ua_availability.html')

    # (overlay panel removed — button opens ua_availability.html directly)

except Exception as _ua_avail_ex:
    print(f'⚠ UA Availability data prep skipped: {_ua_avail_ex}')
    _ua_stations_data = []
    _ua_stations_json = '[]'
...
#  Full-page radiosonde reporting availability map — separate URL
#  Button in synoptic_map.html opens this via window.open('ua_availability.html','_blank')
# ══════════════════════════════════════════════════════════════════════════════
try:
    # Reuse _ua_stations_data / _ua_stations_json already built above.
    # Fall back to rebuilding if that block failed.
    import json as _uasa_json

    if not _ua_stations_data:
        raise RuntimeError('_ua_stations_data is empty — UA data prep failed earlier')

    # ── reuse station data already built above ────────────────────────────────
    # _ua_stations_data contains {lat,lon,c_L,c_R,cid,popup,lbl_L,lbl_R,
    #                              p_L,t_L,w_L,p_R,t_R,w_R}
    # The standalone page uses the same sa* prefixed popup/chart IDs.
    _uasa_json_str = _ua_stations_json   # already JSON-serialised
    _uasa_total    = len(_ua_stations_data)

    # summary counts (both-ok / partial / both-missing)
    _sc2 = lambda c: 2 if c == "#27ae60" else (1 if c == "#2980b9" else 0)
    _uasa_ok = _uasa_rec = _uasa_miss = 0
    for _d in _ua_stations_data:
        _sl, _sr = _sc2(_d["c_L"]), _sc2(_d["c_R"])
        if   _sl == 2 and _sr == 2: _uasa_ok   += 1
        elif _sl == 0 and _sr == 0: _uasa_miss += 1
        else:                       _uasa_rec  += 1
